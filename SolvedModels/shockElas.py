...
def createMatrix(upperLims, lowerLims, S, N, dVec, increVec, stateMat, model,bc, betterCP):
    ...
    muG = np.matrix(model['muG'](stateMat))
    sigmaG = np.matrix(model['sigmaG'](stateMat))
    T = model['T']
    dt = model['dt']
    ...
    levelCoefs = -1.0 / model['dt'] + muG.reshape([S,1]) + 0.5 * np.transpose(np.matrix((np.linalg.norm(sigmaG, axis=1) ** 2)))

    firstCoefs = np.matrix(np.zeros((S,N)))
    secondCoefs = np.matrix(np.zeros((S,N)))
    for n in range(0,N):
        firstCoefs[:,n] = muX[:,n] + np.multiply(sigmaX[n],sigmaG).sum(axis=1)
...
def solvePhi(upperLims, lowerLims, S, N, numShocks, dVec, increVec, stateMat, model, bc, usePardiso, iparms, betterCP):
    ...
    phi0   = np.concatenate([np.ones([S,1]), np.asarray(model['sigmaG'](stateMat))], axis = 1)
    solve  = None
    pSolve = None
...
def computeElasSub(stateMat, model, bc, x0, usePardiso, iparms, betterCP):
    ...
    numShocks = (1 if len(model['sigmaG'](x0).shape) == 1 else model['sigmaG'](x0).shape[1]) ###number of shocks
    ...
    for i in range(numStarts):
        firstType[i,:,:] = np.concatenate([ (func(x0[i])) for func in model['sigmaX']]).T * np.matrix(derivs[i,:,:]) + model['sigmaG'](x0[i]).T
...
def computeElas(stateMat, model, bc, x0, usePardiso = False, iparms = {}, betterCP = True):
    ...
    modelCopy['muG'] = (lambda f1, f2: lambda x: f1(x) + f2(x))(model['muG'], model['muS'])
    modelCopy['sigmaG'] = (lambda f1, f2: lambda x: f1(x) + f2(x))(model['sigmaG'], model['sigmaS'])
    ...
    return expoElas, priceElas, linSys1, linSys2
