                ...
                do
                    ...
                    do
                        ...
                        do
                            ...
                            touch ./bash/$domain_folder/$model_folder/ela24.sh
                            tee ./bash/$domain_folder/$model_folder/ela24.sh << EOF
#! /bin/bash

#SBATCH --account=pi-lhansen
#SBATCH --job-name=ela24
#SBATCH --output=./job-outs/$domain_folder/$model_folder/ela24.out
#SBATCH --error=./job-outs/$domain_folder/$model_folder/ela24.err
...
python3 /project/lhansen/mfrSuiteidplots/SolvedModels/preload24.py --chiUnderline ${chiUnderline} --a_e ${a_e} --a_h ${a_h} --gamma_e ${gamma_e} --gamma_h ${gamma_h} --psi_e ${psi_e} --psi_h ${psi_h} \
                                                    --nV ${nV} --nVtilde ${nVtilde} --V_bar ${V_bar} --Vtilde_bar ${Vtilde_bar} --sigma_V_norm ${sigma_V_norm} --sigma_Vtilde_norm ${sigma_Vtilde_norm} \

EOF
                            sbatch ./bash/$domain_folder/$model_folder/ela24.sh
                        done
                    done
                done
            done
