                ...
                do
                    ...
                    do
                        ...
                        do
                            ...
                            touch ./bash/$domain_folder/$model_folder/mgw160year.sh
                            tee ./bash/$domain_folder/$model_folder/mgw160year.sh << EOF
#! /bin/bash

#SBATCH --account=pi-lhansen
#SBATCH --job-name=mgw160year
#SBATCH --output=./job-outs/$domain_folder/$model_folder/mgw160year.out
#SBATCH --error=./job-outs/$domain_folder/$model_folder/mgw160year.err
...
python3 /project/lhansen/mfrSuiteidplots/SolvedModels/preload_yearr_mgw160.py --chiUnderline ${chiUnderline} --a_e ${a_e} --a_h ${a_h} --gamma_e ${gamma_e} --gamma_h ${gamma_h} --psi_e ${psi_e} --psi_h ${psi_h} \
                                                    --nV ${nV} --nVtilde ${nVtilde} --V_bar ${V_bar} --Vtilde_bar ${Vtilde_bar} --sigma_V_norm ${sigma_V_norm} --sigma_Vtilde_norm ${sigma_Vtilde_norm} \

EOF
                            sbatch ./bash/$domain_folder/$model_folder/mgw160year.sh
                        done
                    done
                done
            done
