...
do
    ...
    do
        ...
        do
            ...
            do
                for gamma_h in 5.0 6.0 7.0 8.0
                do
                    for psi_e in 1.0
                    do
                        for psi_h in 1.0
                        do
                            ...
                            touch ./bash/$domain_folder/$model_folder/elaquarter.sh
                            tee ./bash/$domain_folder/$model_folder/elaquarter.sh << EOF
#! /bin/bash

#SBATCH --account=pi-lhansen
#SBATCH --job-name=elaquarter
#SBATCH --output=./job-outs/$domain_folder/$model_folder/elaquarter.out
#SBATCH --error=./job-outs/$domain_folder/$model_folder/elaquarter.err
...
python3 /project/lhansen/mfrSuiteidplots/SolvedModels/preloadquarter.py --chiUnderline ${chiUnderline} --a_e ${a_e} --a_h ${a_h} --gamma_e ${gamma_e} --gamma_h ${gamma_h} --psi_e ${psi_e} --psi_h ${psi_h} \
                                                    --nV ${nV} --nVtilde ${nVtilde} --V_bar ${V_bar} --Vtilde_bar ${Vtilde_bar} --sigma_V_norm ${sigma_V_norm} --sigma_Vtilde_norm ${sigma_Vtilde_norm} \

EOF
                            sbatch ./bash/$domain_folder/$model_folder/elaquarter.sh
                        done
                    done
                done
            done
