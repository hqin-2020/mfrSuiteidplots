                ...
                do
                    ...
                    do
                        ...
                        do
                            ...
                            touch ./bash/$domain_folder/$model_folder/nb111year.sh
                            tee ./bash/$domain_folder/$model_folder/nb111year.sh << EOF
#! /bin/bash

#SBATCH --account=pi-lhansen
#SBATCH --job-name=nb111year
#SBATCH --output=./job-outs/$domain_folder/$model_folder/nb111year.out
#SBATCH --error=./job-outs/$domain_folder/$model_folder/nb111year.err
...
python3 /project/lhansen/mfrSuiteidplots/SolvedModels/preloadnb111year.py --chiUnderline ${chiUnderline} --a_e ${a_e} --a_h ${a_h} --gamma_e ${gamma_e} --gamma_h ${gamma_h} --psi_e ${psi_e} --psi_h ${psi_h} \
                                                    --nV ${nV} --nVtilde ${nVtilde} --V_bar ${V_bar} --Vtilde_bar ${Vtilde_bar} --sigma_V_norm ${sigma_V_norm} --sigma_Vtilde_norm ${sigma_Vtilde_norm} \

EOF
                            sbatch ./bash/$domain_folder/$model_folder/nb111year.sh
                        done
                    done
                done
            done
